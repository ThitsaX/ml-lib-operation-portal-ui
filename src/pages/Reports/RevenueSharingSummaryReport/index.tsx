...
import { downloadFile, getSettlementIds, generateRevenueSharingSummaryReport } from '@services/report';
...
const initialFileName = 'RevenueSharingSummaryReport';
...
const RevenueSharingSummaryReport = () => {
  ...
  const onDownloadClick = async () => {
    ...
    try {
      const res = await generateRevenueSharingSummaryReport(user, {
  ...
  return (
    <VStack align="flex-start" w="full" h="full" p="3" mt={10}>
      <Stack>
        <Heading fontSize="2xl" fontWeight="bold" mb={6}>{t('ui.revenue_sharing_summary_report')}</Heading>
      </Stack>
...
export default memo(RevenueSharingSummaryReport);
